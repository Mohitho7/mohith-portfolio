...
export default function HeroSection({ heroData }: { heroData: any }) {
  ...
  const roles: string[] = (
    heroData?.identityStatement ||
    "Full-Stack Developer,UI/UX Enthusiast,Problem Solver,Open Source Contributor"
  )
    .split(",")
    .map((s: string) => s.trim())
    .filter(Boolean);
      ...
      >
        ...
        >
          ...
          <span
            style={{
              background:
                "linear-gradient(135deg, #d4af37 0%, #f5e27a 45%, #b8922a 100%)",
          ...
          animate={
            mounted
              ? {
                  opacity: 1,
                  y: 0,
                  transition: { duration: 0.72, delay: 0.44, ease: EASE },
                }
              : {}
          }
          style={{
        ...
        >
          {heroData?.valueProposition ||
            "I design and build products that feel inevitable. From first sketch to deployed product — end-to-end, with intention."}
        </motion.p>
        ...
        >
          <Link
            href={getSafeNavigationHref(heroData?.ctaPrimaryLink, "/#projects")}
            className="btn-primary"
            style={{ padding: "13px 30px", fontSize: "0.92rem" }}
          >
            {heroData?.ctaPrimaryText || "See My Work"}
          </Link>
          <Link
            href={getSafeNavigationHref(
              heroData?.ctaSecondaryLink,
              "/#contact",
            )}
